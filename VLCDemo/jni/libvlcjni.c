...
static void vlc_event_callback(const libvlc_event_t *ev, void *data)
{
    ...
    if (ev->type == libvlc_MediaPlayerPositionChanged) {
        ...
        (*env)->DeleteLocalRef(env, sData);
    } else if(ev->type == libvlc_MediaPlayerRecordableChanged) {

    } else if(ev->type == libvlc_MediaPlayerRecordingFinished) {
    	jstring sData = (*env)->NewStringUTF(env, "data");
    	jstring file_location = (*env)->NewStringUTF(env, ev->u.media_player_recording_finished.psz_filename);
    	(*env)->CallVoidMethod(env, bundle, putString, sData, file_location);
    	(*env)->DeleteLocalRef(env, sData);
    	(*env)->DeleteLocalRef(env, file_location);
...
// added for stun start
void Java_org_videolan_libvlc_LibVLC_setRtspOverTcp(JNIEnv *env, jobject thiz,
                                       jlong instance, jboolean rtspOverTcp)
{
	libvlc_media_player_t *mp = getMediaPlayer(env, thiz);
	if (mp) {
		libvlc_media_player_set_rtsp_tcp(mp, rtspOverTcp);
	}
}

void Java_org_videolan_libvlc_LibVLC_setUseStun(JNIEnv *env, jobject thiz,
                                       jlong instance, jboolean useStun)
{
	libvlc_media_player_t *mp = getMediaPlayer(env, thiz);
	if (mp) {
		libvlc_media_player_set_use_stun(mp, useStun);
	}
}
void Java_org_videolan_libvlc_LibVLC_setStunServerPort(JNIEnv *env, jobject thiz,
                                       jlong instance, jint stunPort)
{
	libvlc_media_player_t *mp = getMediaPlayer(env, thiz);
	if (mp) {
		libvlc_media_player_set_stun_server_port(mp, stunPort);
	}
}
void Java_org_videolan_libvlc_LibVLC_setStunServerAddr(JNIEnv *env, jobject thiz,
        jlong instance, jstring stunServerAddr) {
	jboolean isCopy;
	libvlc_media_player_t *mp = getMediaPlayer(env, thiz);
    const char *psz_stun_server_addr = (*env)->GetStringUTFChars(env, stunServerAddr, &isCopy);
	if (mp) {
		libvlc_media_player_set_stun_server_addr(mp, psz_stun_server_addr);
	}
    (*env)->ReleaseStringUTFChars(env, stunServerAddr, psz_stun_server_addr);
...
static void create_player_and_play(JNIEnv* env, jobject thiz,
    ...
    static const libvlc_event_type_t mp_events[] = {
        ...
        libvlc_MediaPlayerRecordableChanged,//add for stun
        libvlc_MediaPlayerRecordingFinished,//add for stun
